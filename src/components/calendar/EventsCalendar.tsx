...
export default function EventsCalendar({
  ...
  const handleActiveStartDateChanged = ({
    action,
    activeStartDate,
    value,
    view,
    ...
    switch (action) {
      ...
      case 'onChange':
        if (
          activeStartDate <= last &&
          activeStartDate >= new Date(first.getFullYear(), first.getMonth())
        ) {
          router.push(
            `/calendar/${activeStartDate.getFullYear()}/${activeStartDate.getMonth()}`,
          );
